...
static void HW_RTC_SetConfig(void)
{
  ...
  if ( RTC_DateStruct.Year == 0 )
  {
    ...
    HAL_RTC_SetTime(&hrtc, &RTC_TimeStruct, FORMAT_BIN);

    RTC_DateStruct.WeekDay = RTC_WEEKDAY_THURSDAY;
    RTC_DateStruct.Month = RTC_MONTH_DECEMBER;
    RTC_DateStruct.Date = 29;
    RTC_DateStruct.Year = 22;
    HAL_RTC_SetDate(&hrtc, &RTC_DateStruct, FORMAT_BIN);
  }
...
{
  RTC_HandleTypeDef *hrtcl = &RtcHandle;
  ...
  if (__HAL_RTC_ALARM_GET_IT_SOURCE(hrtcl, RTC_IT_ALRA) != RESET)
  {
    /* Get the pending status of the AlarmA Interrupt */
    if (__HAL_RTC_ALARM_GET_FLAG(hrtcl, RTC_FLAG_ALRAF) != RESET)
    {
      /* Clear the AlarmA interrupt pending bit */
      __HAL_RTC_ALARM_CLEAR_FLAG(hrtcl, RTC_FLAG_ALRAF);
      /* AlarmA callback */
      HAL_RTC_AlarmAEventCallback(hrtcl);
    }
  }
}
