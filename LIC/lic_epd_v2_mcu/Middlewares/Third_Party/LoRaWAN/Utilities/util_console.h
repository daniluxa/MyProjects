...
#define PPRINTF(...)     do{ } while( 0!= DIAG_MSG(__VA_ARGS__) ) //Polling Mode

#define PRINTF(...)     do{  DIAG_MSG(__VA_ARGS__); }while(0)
#define PRINTNOW()      do{                                                           \
                          SysTime_t stime  =SysTimeGetMcuTime();                      \
                          DIAG_MSG("%3ds%03d: ",stime.Seconds, stime.SubSeconds); \
                         }while(0) 

#define TVL1(X)    do{ X }while(0);
#define TVL2(X)    do{ X }while(0);
